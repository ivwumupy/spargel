...
public:
    explicit Renderer(spargel::ui::window* window,
        ...
        _shader = _device->createShaderLibrary(
            spargel::base::make_span<u8>(blob->size(), (u8*)blob->map_data()));
        ...
        builder.addVertexAttribute(vertex_attribute_format::float32x2, 0, 0, 0);
    ...
    void on_render() override {
        ...
        auto encoder = cmdbuf->beginRenderPass(
            {.color_attachments = {{.texture = texture, .clear_color = {0.0, 1.0, 1.0, 1.0}}}});
