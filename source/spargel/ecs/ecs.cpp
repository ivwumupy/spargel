...
namespace spargel::ecs {
    ...
    world_id create_world() {
        struct world* world =
            (struct world*)base::default_allocator()->allocate(
                sizeof(struct world));
     ...
     */
    static void grow_array(void** ptr, ssize* capacity, ssize stride,
                           ssize need) {
     ...
     */
    static ssize find_archetype(world_id world, ssize count,
                                component_id const* ids) {
        ssize result = -1;
        for (ssize i = 0; i < world->archetype_count; i++) {
            struct archetype* archetype = &world->archetypes[i];
    ...
    static ssize create_archetype(world_id world, ssize component_count,
        ...
        archetype->component_ids =
            (component_id*)base::default_allocator()->allocate(
                sizeof(component_id) * component_count);
        memcpy(archetype->component_ids, component_ids,
               sizeof(component_id) * component_count);
        archetype->components = (void**)base::default_allocator()->allocate(
            sizeof(void*) * component_count);
    ...
    int spawn_entities(world_id world, struct spawn_descriptor* desc,
        ...
        if (archetype_id < 0) {
            archetype_id = create_archetype(world, desc->component_count,
                                            desc->components);
        }
        struct archetype* archetype = &world->archetypes[archetype_id];
        if (archetype->col_count + desc->entity_count >
            archetype->col_capacity) {
    ...
    int query(world_id world, struct query_descriptor* desc,
              struct view* view) {
        ssize archetype_id =
    ...
    static void delete_in_archetype(world_id world, struct archetype* archetype,
                                    ssize index) {
