...
TEST(Json_Codec_Encode_Array) {
    ...
    }
    {
        ...
        v2.push(-4);
        v.push(base::move(v2));
        auto result = makeArrayCodec(makeArrayCodec(I32Codec<B>{})).encode(encodeBackend, base::move(v));
        spargel_check(result.isLeft() && result.left().type == JsonValueType::array);
...
TEST(Json_Codec_Decode_Array) {
    {
        ...
        auto result = makeArrayCodec(U32Codec<B>{}).decode(decodeBackend, base::move(result_json.left()));
        spargel_check(result.isLeft());
    ...
    }
    {
        ...
        auto result = makeArrayCodec(makeArrayCodec(StringCodec<B>{})).decode(decodeBackend, base::move(result_json.left()));
        spargel_check(result.isLeft());
