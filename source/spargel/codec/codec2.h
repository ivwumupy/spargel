...
class RecordEncoder : public Encoder2<EB, S> {
 ...
 public:
  ...
  base::unique_ptr<Encoder2<EB, S>> clone() const override {
    return base::apply([](const Builders&... builders) { return base::make_unique<RecordEncoder>(builders...); }, _builders);
  }
...
class RecordDecoder : public Decoder2<DB, S> {
 ...
 public:
  ...
  base::Either<S, ErrorType> decode(DB& backend, const DataType& data) const override {
    return decodeImpl(backend, data, base::index_sequence_for<Decoders...>{});
  }

 private:
  F _func;
  base::tuple<base::unique_ptr<typename Decoders::base_type>...> _decoders;

  template <typename T, T... indices>
  base::Either<S, ErrorType> decodeImpl(DB& backend, const DataType& data, base::integer_sequence<T, indices...>) const {
    base::Optional<ErrorType> error;
    base::tuple<base::Optional<typename Decoders::type>...> values;

    bool success = ([&]() {
      const auto i = indices;
      auto& decoder = base::get<i>(_decoders);
