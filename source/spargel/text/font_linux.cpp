...
namespace spargel::text {
    ...
    FontLinux::FontLinux(FontManagerLinux* font_manager, base::Span<u8> buffer,
                         float size)
        ...
        initFreeType();
        {
            char *family_name = ft_face->family_name,
                 *style_name = ft_face->style_name;
...
#error "unimplemented"
    ...
    Bitmap FontLinux::rasterizeGlyph(GlyphId id, float scale) {
        ...
        if (FT_Load_Glyph(ft_face, glyph_index, FT_LOAD_DEFAULT) != 0) {
            spargel_log_fatal("Error calling FT_Load_Glyph (glyph_index=%d).",
                              glyph_index);
            spargel_panic_here();
        }
        ...
        if (FT_Render_Glyph(slot, FT_RENDER_MODE_NORMAL) != 0) {
            spargel_log_fatal("Error calling FT_Render_Glyph (glyph_index=%d).",
                              glyph_index);
            spargel_panic_here();
        }

        // FIXME
        spargel_check(slot->bitmap.pitch > 0 &&
                      slot->bitmap.width == (u32)slot->bitmap.pitch);
...
#define UNIT2POINT(n) ((float)((n) * size) / units_per_EM)
        return GlyphInfo{
            .bounding_box =
                math::Rectangle{
                    .origin =
                        math::Vector2f{.x = UNIT2POINT(metrics.horiBearingX),
                                       .y = UNIT2POINT(metrics.horiBearingY -
                                                       metrics.height)},
                    .size =
                        math::RectangleSize{
                            .width = UNIT2POINT(metrics.width),
...
#undef UNIT2POINT
    ...
    void FontLinux::initFreeType() {
        ...
        auto error = FT_New_Memory_Face(font_manager->ft_library, buffer.data(),
                                        buffer.count(), 0, &ft_face);
        if (error) {
            if (error == FT_Err_Unknown_File_Format) {
                spargel_log_fatal(
                    "Error calling FT_New_Memory_Face: Unsupported file "
                    "format.");
            } else {
    ...
    void FontLinux::initHarfBuzz() {
        ...
        hb_blob = hb_blob_create(reinterpret_cast<const char*>(buffer.data()),
                                 buffer.count(), HB_MEMORY_MODE_READONLY,
                                 nullptr, nullptr);
        if (!hb_blob) {
