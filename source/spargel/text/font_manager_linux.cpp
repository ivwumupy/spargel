...
namespace spargel::text {
    ...
    base::UniquePtr<Font> FontManagerLinux::createDefaultFont() {
        auto default_font_filepath =
            "/usr/share/fonts/truetype/msttcorefonts/Times_New_Roman.ttf";

        if (!default_font_file.hasValue()) {
            auto resource = resource_manager.open(
                resource::ResourceId(default_font_filepath));
            if (!resource.hasValue()) {
                spargel_log_fatal("Unable to open default font: %s.",
        ...
        return base::makeUnique<FontLinux>(this,
                                           default_font_file.value()->getSpan(),
